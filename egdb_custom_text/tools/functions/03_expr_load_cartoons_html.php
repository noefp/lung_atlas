...
$cartoons_data=[$data1["cartoons"],$data2["cartoons"],$data3["cartoons"],$data4["cartoons"]];

$jcartoons=[];
$canvas_w = [];
$canvas_h=[];
$titles=["Single Cell Ctrl-ARDS","Bulk RNA-seq ARDS","Bulk RNA-seq SARSCOV2","Proteomics"];
// $titles_color=['blue','green','orange'];
$cartoons=[];

$myCanvas=["myCanvas","myCanvas2","myCanvas3","myCanvas4"];
$canvas_div=["canvas_div","canvas_div1","canvas_div2","canvas_div3"];
$cartoon_labels=["cartoon_labels1","cartoon_labels2","cartoon_labels3","cartoon_labels4"];
$sel_cartoons=["sel_cartoons1","sel_cartoons2","sel_cartoons3","sel_cartoons4"];
$kj_image=["_kj_image","_kj_image1","_kj_image2","_kj_image3"];
$cartoon_load=[];
...
<script>
  ...
  var canvas_w = [];
  var cartoon=[];
  var myCanvas=["myCanvas","myCanvas2","myCanvas3","myCanvas4"];
  var gene_list = [];
  ...
  if(cartoon_load[3]){
    imgObj[3] = <?php echo json_encode($jcartoons[3]) ?>;
    canvas_h[3] = <?php echo json_encode($canvas_h[3]) ?>;
    canvas_w[3] = <?php echo json_encode($canvas_w[3]) ?>;
    cartoons_all_genes[3]= <?php echo $cartoons_data[3] ?>;
    gene_list[3]= <?php echo json_encode(array_keys(json_decode($cartoons_data[3],true))) ?>;
  }
...
cartoon_load.forEach(load =>
 ...
 // ######################################################## Cartoons gene selection
  ...
  var image_id,parent;
    ...
    if(parent=="cartoon_labels4")
    {
      //  alert("image: "+image_id+" | "+parent);
      obj= canvas[3].get('#'+image_id)[0];
      color_rgb=[obj.attrs.red,obj.attrs.green,obj.attrs.blue];
      obj.cache();
      obj.filters([Kinetic.Filters.RGB]);
      obj.red(150).green(150).blue(150);
  ...
  $( "#sel_cartoons4" ).change(function() {
    //alert( this.value );
    //cartoon_one_gene = cartoons_all_genes[this.value];
    
    cartoon_active_gene = $('#sel_cartoons4').val();
    
    // alert("Cartoon gene: "+cartoon_active_gene);
    // alert("genelist: "+gene_list[0].findIndex((element) => element == cartoon_active_gene))
    cartoon_gene_select_index = gene_list[3].findIndex((element) => element == cartoon_active_gene);
    
    draw_gene_cartoons(canvas[3],imgObj[3],cartoons_all_genes[3],gene_list[3][cartoon_gene_select_index],3,ranges_array[3],colors_array[3]);
    
    gene_expr_values = cartoons_all_genes[3][cartoon_active_gene];
    ...
    for (var sample in gene_expr_values){
      // alert(sample)
      expr_value = gene_expr_values[sample];
      sample_id=sample+"_kj_image3";    
      color_rgb=get_expr_color(expr_value,ranges_array[3],colors_array[3]);
      // $(document.getElementById(sample_id)).html(sample+": "+expr_value).css('color','rgb('+color_rgb+')');
      $(document.getElementById(sample_id)).html(sample+": "+expr_value).css('text-decoration','double underline').css('text-decoration-color','rgb('+color_rgb+')');
      // $(document.getElementById(sample_id)).html(sample+": "+expr_value);
    }
...
crearFila(colors_array[3],ranges_text_array[3],'color-table3');
