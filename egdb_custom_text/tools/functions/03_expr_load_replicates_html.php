   ...
   <center>
  <?php
  $replicates_data=[$data1["replicates"],$data2["replicates"],$data3["replicates"],$data4["replicates"]];
    // $replicates_data=[$data1["replicates"]];

  $found_genes=[];
  $name_table=['Single Cell Ctrl-ARDS','Bulk RNA-seq ARDS','Bulk RNA-seq SARSCOV2',"Proteomics"];
    // $name_table=['Single Cell Ctrl-ARDS'];
    ...
    foreach($replicates_data as $index => $replicate)
    {
      ...
      echo"<div id=\"chart_rep_frame$index\" style=\"border:2px solid #666; padding-top:7px\">"; 
          ...
          echo "<select class=\"form-control sel1\" id=\"sel1$index\">";
            ...
            echo"</select>";
          echo"</div>";
          echo"<div id=\"chart_rep$index\"></div>";
          echo"</div>";
      }
...
<script type="text/javascript">
  ...
  sample_array[3] = <?php echo json_encode($data4["header"],true) ?>;
  replicates_all_gene[3] = <?php  echo json_encode($data4["replicates"]); ?>;
  samples_found[3] = <?php echo json_encode($found_genes[3]) ?>;
